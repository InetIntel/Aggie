...
export function IncidentOverallStatus({
  ...
  if (publication_status.includes("Shared with Networks")) {
    return (
      <p className={`bg-lime-200 ${className}`} {...props}>
        Shared with Networks
      </p>
    );
  } else if (publication_status.includes("Published")) {
    return (
      <p className={`bg-green-200 ${className}`} {...props}>
        Published
      </p>
    );
  }
  ...
  switch (confirmation_status) {
    ...
    case "false":
      return (
        <p className={`bg-red-200 ${className}`} {...props}>
          Unable to Confirm
        </p>
      );
    default:
  ...
  switch (verification_status) {
    ...
    case "true":
      return (
        <p className={`bg-amber-200 ${className}`} {...props}>
          Confirming
        </p>
      );
    case false:
...
export function IncidentStatuses({
  ...
  const verified = (
    ...
    ? <span className={`bg-amber-200 ${className}`} {...props}>Verifying</span>
    : verification_status === "true" || verification_status === true
      ? <span className={`bg-green-200 ${className}`} {...props}>Verified</span>
      : verification_status === "false" || verification_status === false
        ? <span className={`bg-red-200 ${className}`} {...props}>Unable to Verify</span>
        : null
  );
  const confirmed = (
    confirmation_status === "maybe" || isNil(confirmation_status)
    ? <span className={`bg-amber-200 ${className}`} {...props}>Confirming</span>
    : confirmation_status === "true" || confirmation_status === true
      ? <span className={`bg-green-200 ${className}`} {...props}>Confirmed</span>
      : confirmation_status === "false" || confirmation_status === false
        ? <span className={`bg-red-200 ${className}`} {...props}>Unable to Confirm</span>
        : null
  );
  const published = (
    publication_status.includes("Published")
    ? <span className={`bg-green-200 ${className}`} {...props}>Published</span>
    : <span className={`bg-red-200 ${className}`} {...props}>Not Published</span>
  );
  const shared = (
